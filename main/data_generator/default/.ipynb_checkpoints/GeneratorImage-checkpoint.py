...
from utils import load_config, dataset_properties, speckle_transform
...
parser.add_argument('--mode', default='tif', help='Supports .tif or .h5')
...
width_scale_p = config['AUGMENTATION']['WIDTH_SCALE_P']
width_scale_A = config['AUGMENTATION']['WIDTH_SCALE_A']

speckle_prob = config['AUGMENTATION']['SPECKLE_PROB']
speckle_alpha = config['AUGMENTATION']['SPECKLE_alpha']
speckle_p = config['AUGMENTATION']['SPECKLE_p']
speckle_sigma = config['AUGMENTATION']['SPECKLE_sigma']
...
# input directory
if args.mode == 'tif':
    mask_files = sorted(glob.glob(args.mask_dir+'mask_*.tif'))
elif args.mode == 'h5':
    mask_files = sorted(glob.glob(args.mask_dir+'mask_*.h5'))
else:
    raise Exception("--mode must be \'tif\' or \'h5\'.")
...
for i in tqdm(range(N), desc='Generating Image Textures'):
    ...
    if np.random.rand() < width_scale_p:

        grid_points = np.random.choice([2**i for i in range(2,5) if 2**i < np.min(mask_labels.shape[1:])/2])
        pN_1 = perlin_optim_3d(npt_seq(mask_labels.shape[1:]), [grid_points,grid_points,grid_points], octaves=1, sf=2, order=1)
        pN_1 = np.exp( width_scale_A*pN_norm( pN_1[:mask_labels.shape[1],:mask_labels.shape[2],:mask_labels.shape[3]]) )
    ...
    if np.random.rand() < speckle_prob:

        image = speckle_transform(image, 
                                  speckle_alpha, 
                                  speckle_p, 
                                  mask=mask_labels[0].astype(bool), 
                                  sigma=speckle_sigma
                                 )
    ...
    if args.mode == 'tif':
        imwrite(f'{args.mask_dir}{args.sub_folder}image_{str(i+1).zfill(5)}.tif', image[np.newaxis].astype(np.float32))
    else:
...
if args.mode == 'tif':
    im_list = glob.glob(f'{args.mask_dir}{args.sub_folder}image_*.tif')
else:
