...
def mesh_with_values(voronoi, values, spacing=(1,1,1), max_distance=10.0, operation='max', mode='inner', step=0.5):
    ...
    #     verts, faces, normals, _ = measure.marching_cubes(gaussian_filter(voronoi==1, sigma=sigma), level=0.5, spacing=spacing)
    # else:
    #     verts, faces, normals, _ = measure.marching_cubes(voronoi==1, level=0.5, spacing=spacing)
    verts, faces, normals, _ = measure.marching_cubes(gaussian_filter((voronoi==1).astype(float), sigma=.5/(np.array(spacing))), 
...
def custom_het_i_calc(image, annotations, spacing=(1.0,1.0,1.0), max_depth=1.0, operation='max', sigma=None):
    '''
    Input a single image frame with annotations.
